...
static UIView * extracted(MTBBarcodeScanner *object) {
	return object.previewView;
}
...
- (BOOL)startScanningWithResultBlock:(void (^)(NSArray<AVMetadataMachineReadableCodeObject *> *codes))resultBlock error:(NSError **)error {
	...
	[extracted(self).layer insertSublayer:self.capturePreviewLayer atIndex:0]; // Insert below all other views
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
	...
	for (AVMetadataObject *metaData in metadataObjects) {
		...
		(AVMetadataMachineReadableCodeObject *)[self.capturePreviewLayer
...
- (void)setupSessionQueue {
	...
	self.privateSessionQueue = dispatch_queue_create("com.phraselock.MTBBarcodeScanner.captureSession", DISPATCH_QUEUE_SERIAL);
}
